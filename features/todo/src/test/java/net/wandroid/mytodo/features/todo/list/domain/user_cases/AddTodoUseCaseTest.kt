...
class AddTodoUseCaseTest {
    ...
    fun `emit Loading before Success result`() = runTest {
        //given
        coEvery { todoRepository.addTodo(any()) } returns mockk(relaxed = true)
        //when
        val results = addTodoUseCase(
            title = "",
            content = "",
        ).toList()
        //then
    ...
    fun `emit Loading before Fail result`() = runTest {
        ...
        coEvery { todoRepository.addTodo(any()) } throws e
        //when
        val results = addTodoUseCase(
            title = "",
            content = "",
    ...
    fun `emit Success with data on success`() = runTest {
        ...
        coEvery { todoRepository.addTodo(any()) } returns todoResponseDto
        //when
        val result = addTodoUseCase(
            title = "title",
            content = "content",
        ...
        Truth.assertThat(result.data).isEqualTo(expected)
    }
}